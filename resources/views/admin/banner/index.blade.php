        ...
        <div class="">
            <div class="panel">
                <div class="panel-heading">
                    <span class="panel-title">Banners</span>
                </div>
                                        ...
                                        @if(count($data) > 0)
                                            @foreach($data as $row)
                                                <tr class="id{{$row->id}}">
                                                    ...
                                                    <td class="">
                                                        @if($row->video)
                                                            <video uk-video uk-cover preload="auto" width="200" height="auto" loop
                                                                playsinline autoplay muted
                                                                data-setup='{"fluid": true,"controls": false,"loop":true}'>
                                                                <source src="{{asset('uploads/banners/' . $row->video)}}"
                                                                    type="video/mp4" muted>
                                                            </video>
                                                        @else
                                                            <img src="{{url(env('PUBLIC_PATH') . 'uploads/banners/' . $row->picture)}}"
                                                                width="200" />
                                                        @endif
                                                    </td>
